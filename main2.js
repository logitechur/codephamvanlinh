...
function rcComments(json) {
  for (let i = 0; i < limCmt; i++) {
    ...
    var time = second < 60 ? second + " giây trước" : second < 3600 ? Math.floor(second / 60) + " phút trước" : second < 60 * 60 * 24 ? Math.floor(second / (60 * 60)) + " giờ truớc" : second < 60 * 60 * 24 * 7 ? Math.floor(second / (60 * 60 * 24)) + " ngày truớc" : Math.floor(second / (60 * 60 * 24 * 7)) + " tuần truớc";
    var comment = {
      author: name,
      avatar: commentJson.author[0].gd$image.src == "https://img1.blogblog.com/img/b16-rounded.gif" ? noAvatar : commentJson.author[0].gd$image.src == "https://img1.blogblog.com/img/blank.gif" ? noAvatar : commentJson.author[0].gd$image.src,
      content: content,
      time: time,
...
document.write(`<script type="text/javascript" src="${blogUri}/feeds/comments/default?alt=json-in-script&max-results=${limCmt}&callback=rcComments"></script>`);
